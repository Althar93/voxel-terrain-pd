...
BitmapPixel bitmap_getPixel(const Bitmap* bitmap, const unsigned int x, const unsigned int y)
{
    const unsigned int index = x + y * bitmap->infoHeader.biWidth;
    return bitmap->data[index];
}

BitmapPixel bitmap_lerpPixel(const BitmapPixel* lhs, const BitmapPixel* rhs, const float factor)
{
    return (BitmapPixel)
    {
        .r = lhs->r + (unsigned char)((rhs->r - lhs->r) * factor),
        .g = lhs->g + (unsigned char)((rhs->g - lhs->g) * factor),
        .b = lhs->b + (unsigned char)((rhs->b - lhs->b) * factor)
    };
}

BitmapPixel bitmap_getPixelLinear(const Bitmap* bitmap, const float x, const float y)
{
    const unsigned int x0   = (unsigned int)floorf(x);
    const unsigned int x1   = x0 >= bitmap->infoHeader.biWidth  - 1 ? x0 : x0 + 1u;
    const unsigned int y0   = (unsigned int)floorf(y);
    const unsigned int y1   = y0 >= (unsigned int)abs(bitmap->infoHeader.biHeight) - 1 ? y0 : y0 + 1u;
